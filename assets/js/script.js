...
function showQuestions(index) {
    ...
    //set onclick attribute to all avaliable options
...
function optionSelected(answer) {
    ...
    if (userAns == correcAns) { //if user selected option is equal to array's correct answer
            ...
            answer.classList.add("correct"); //adding green color to the correct selected option
            answer.insertAdjacentHTML("beforeend", tickIconTag); //adding tick icon to correct selected option
            console.log("Resposta Correta");
            console.log("Suas respostas corretas = " + userScore);
    } else {
            answer.classList.add("incorrect"); //adding red color to the incorrect selected option
            answer.insertAdjacentHTML("beforeend", crossIconTag); //adding cross icon to the incorrect options selected
...
function startTimer(time) {
    ...
    function timer() {
        ...
        if(time < 9) { //if time is less then 9
        ...
        if(time < 0) { //if timer is less than 0
            ...
            let correcAns = questions[que_count].answer; //getting correct answer from array
            for(i=0; i < allOptions; i++) {
                if(option_list.children[i].textContent == correcAns) { //if there is an option which is macthed to an array answer
                    option_list.children[i].setAttribute("class", "option corret"); //adding green color to matched option
                    option_list.children[i].insertAdjacentHTML("beforeend", tickIconTag); //adding tick icon to matched option
                    console.log("Tempo esgotado: Resposta correta selecionada autommaticamente.");
                }
            }
            for(i=0; i < allOptions; i++) {
                option_list.children[i].classList.add("disabled"); //once user select an option then disabled all options
